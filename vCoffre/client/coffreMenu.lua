...
local function getCoffreItems(coffreSelected)
    ESX.TriggerServerCallback("vCoffre:coffreItems", function(items)
        coffreInventory = {}
        for i = 1, #items, 1 do
            local itemsStock = items[i]
            if itemsStock.count > 0 then
                table.insert(coffreInventory, {
                    label = itemsStock.label,
                    nombre = itemsStock.count,
                    name = itemsStock.name,
                })
            end
        end
    end, coffreSelected)
...
function openCoffreMenu(coffreSelected)
    ...
    if coffreSelected.coffreItem then
        getCoffreItems(coffreSelected.addonInventory)
    end
                    ...
                    onSelected = function()
                        ...
                        if value ~= nil then
                            if value <= v.nombre then
                                serverInterrac = true
                                TriggerServerEvent("vCoffre:setItems", coffreSelected, "deposit", v.name, value)
                                getPlayerJob()
                                getCoffreItems(coffreSelected.addonInventory)
                            else
                ...
                RageUI.Button(traduction("button_deposit_item_selected_name"):format(v.label), nil, {RightLabel = traduction("rightlabel_total"):format(v.nombre)}, not serverInterrac, {
                    onSelected = function()
                        ...
                        if value ~= nil then
                            if value <= v.nombre then
                                serverInterrac = true
                                TriggerServerEvent("vCoffre:setItems", coffreSelected, "recup", v.name, value)
                                getPlayerJob()
                                getCoffreItems(coffreSelected.addonInventory)
                            else
                                ESX.ShowNotification(traduction("item_selected_sup"))
