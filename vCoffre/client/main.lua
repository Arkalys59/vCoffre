...
Keys.Register("f9", "f9", "Menu Création Coffre", function()
    ESX.TriggerServerCallback("vCoffre:getPlayerGroup", function(group)
        if not Config.option.getAllowed[group] then
            ESX.ShowNotification(traduction("not_permission"))
            return
        end
        TriggerEvent("vCoffre:openMenu")
    end)
end)
