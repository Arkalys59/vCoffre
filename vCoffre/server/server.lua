...
ESX.RegisterServerCallback("vCoffre:getPlayerGroup", function(source, cb)
...
ESX.RegisterServerCallback("vCoffre:coffreItems", function(source, cb, Society)
...
RegisterCommand(Config.option.commandName, function(src)
    if src == 0 then return end
    local xPlayer = ESX.GetPlayerFromId(src)

    if not Config.option.getAllowed[xPlayer.group] then
...
RegisterNetEvent("vCoffre:setItems")
AddEventHandler("vCoffre:setItems", function(coffreSelected, actionType, itemName, value)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
